...
func (this *EtlController) Prepare() {
	...
	if this.CheckSign() == false { // 校验api_sign
		this.RenderApiJsonEmpty(apicode.SignError, apicode.Msg(apicode.SignError))
		return
	}
}
...
func (this *EtlController) Upload() {
	...
	*/
	...
	//消息在处理
	if mm.CheckMsgProcessing(params.Api_sign) {
		this.RenderApiJsonEmpty(apicode.MsgProcessing, apicode.Msg(apicode.MsgProcessing))
		return
	}

	//消息已经处理完成
	if mm.CheckMsgProcessed(params.Api_sign) {
		this.RenderApiJsonEmpty(apicode.MsgUploaded, apicode.Msg(apicode.MsgUploaded))
		return
	}
	...
	// 消息已经上传
	if data := mm.HasReceivedMessage(msg.Rs_key); data > 0 {
		this.RenderApiJsonSlice(apicode.ReceivedMsg, apicode.Msg(apicode.ReceivedMsg), data)
		return
	}

	mm.SetMsgProcessing(params.Api_sign) //设置Msg正在处理标志

	// 插入消息
	if insertId, err := mm.InsertMessage(msg); err == nil && insertId > 0 { // 插入成功
		mm.SetMsgProcessed(params.Api_sign) //设置标志表示消息处理完成
		data := map[string]int64{"id": insertId}
		this.RenderApiJsonSlice(apicode.Success, apicode.Msg(apicode.Success), data)
	} else { // 插入失败
		mm.DelMsgProcess(params.Api_sign) //清除设置的消息处理标志
		jsonStr, _ := json.Marshal(msg)
		utils.Log.Error("插入消息失败, msg: " + string(jsonStr) + ", err:" + err.Error()) // 记录log
		this.RenderApiJsonEmpty(apicode.MsgInsertFailed, apicode.Msg(apicode.MsgInsertFailed))
	}
}
...
func (this *EtlController) Operate() {
	...
	if err != nil { // 解析metadata数据失败
		...
		this.RenderApiJsonEmpty(apicode.ParseMetadataFailed, apicode.Msg(apicode.ParseMetadataFailed))
		return
	}
	...
	if insertId, err := om.InsertOperate(params, operateMetadata); err == nil && insertId > 0 { // 插入成功
		...
		this.RenderApiJsonEmpty(apicode.InsertOpraterFailed, apicode.Msg(apicode.InsertOpraterFailed))
	}
}
...
func (this *EtlController) GetTime() {
	...
	if nil == err1 {
		...
		this.RenderApiJsonEmpty(apicode.InsertStyleFailed, apicode.Msg(apicode.InsertStyleFailed))
	}
}
